// Nemvai / ArenaX — Dashboard (Secure, textContent only, no innerHTML injection)
...
async function loadMe(){
  try {
    ...
    if (userLine && me.user) {
      userLine.textContent = `مرحباً ${me.user.username} • ${me.user.email}`;
    }
  } catch {
    location.href = "/";
  }
...
async function loadTasks(){
  ...
  try {
    ...
    const progTasks = document.getElementById("progTasks");
    if (progTasks) progTasks.style.width = pct + "%";

    if (!tasks.length) {
      const empty = document.createElement("div");
      empty.className = "hint";
      empty.textContent = "لا توجد مهام — أضف أول مهمة ✨";
...
async function createTask(e){
  ...
  try {
    ...
    // ✅ إعادة ضبط الفلتر لتظهر المهمة الجديدة فوراً في القائمة
...
async function loadHabits(){
  try {
    ...
    if (statStreak) {
      statStreak.textContent = habits.length ? `إجمالي الستريك: ${totalStreak} 🔥` : "";
    }
    ...
    habits.forEach(h => {
      ...
      iconBox.style.background = (h.color || "#8b5cf6") + "22";
      iconBox.style.borderColor = (h.color || "#8b5cf6") + "55";
...
async function createHabit(e){
  ...
  const iconEl = document.getElementById("h_icon");
  const colorEl = document.getElementById("h_color");

  const payload = {
    name: nameEl ? nameEl.value.trim() : "",
    icon: (iconEl && iconEl.value.trim()) ? iconEl.value.trim() : "🔥",
    color: colorEl ? colorEl.value : "#8b5cf6"
  };
...
// ربط الفلتر بحدث التغيير
const filterStatusEl = document.getElementById("filterStatus");
if (filterStatusEl) {
  filterStatusEl.onchange = () => loadTasks();
}
